...
<head>
  ...
  <style>
    ...
    *{{box-sizing:border-box}}html{{scroll-behavior:smooth}}body{{margin:0;background:var(--bg);color:#102a43;font-family:"Segoe UI",Arial,"Microsoft YaHei",sans-serif;line-height:1.55}}a{{color:inherit;text-decoration:none}}button,input{{font-family:inherit}}
    ...
    .radarBox{{min-width:0}}.radarHead{{display:flex;justify-content:space-between;gap:10px;align-items:center}}.radarCurrent{{font-size:12px;color:var(--blue);font-weight:900}}.radarSvg{{display:block;margin:6px auto;max-width:100%;height:360px}}.radarSvg.small{{height:300px}}.radar-grid{{fill:none;stroke:#d9e6f2;stroke-width:1}}.radar-axis{{stroke:#d9e6f2;stroke-width:1}}.radar-label{{font-size:12px;font-weight:800;fill:#0b3155;text-anchor:middle;dominant-baseline:middle}}.radar-series{{fill:var(--series-color);fill-opacity:.08;stroke:var(--series-color);stroke-width:2.3;transition:.18s}}.factorRadar .radar-series{{fill:none;stroke:none}}.factorRadar .radar-segment{{stroke:var(--series-color);stroke-width:2.3;stroke-linecap:round;opacity:.92;vector-effect:non-scaling-stroke}}.factorRadar .radar-dot{{fill:#fff;stroke:var(--series-color);stroke-width:1.8;vector-effect:non-scaling-stroke}}.radarBox.focus .radar-series,.factorRadar.focus .radar-series{{opacity:.08}}.radarBox.focus .radar-series.active,.factorRadar.focus .radar-series.active{{opacity:1;fill-opacity:.18;stroke-width:3.8}}.factorRadar.focus .radar-series.active .radar-segment{{stroke-width:3.5}}.factorRadar.focus .radar-series.active .radar-dot{{stroke-width:2.6}}.radarLegend{{display:flex;flex-wrap:wrap;gap:6px;justify-content:center}}.radarLegend button{{border:1px solid #bfd0e0;background:#fff;border-radius:4px;padding:5px 7px;font-size:12px;cursor:pointer;font-weight:700}}.radarLegend i{{display:inline-block;width:10px;height:10px;margin-right:5px;border-radius:2px}}.radarLegend button:hover{{border-color:var(--yellow);box-shadow:0 0 0 2px rgba(245,180,0,.18)}}.radarLegend.compact button{{font-size:11px;padding:4px 6px}}
...
<script>
function normalizeFactorRadars(){{
  const ns='http://www.w3.org/2000/svg';
  document.querySelectorAll('.factorRadar svg polygon.radar-series').forEach(poly=>{{
    ...
    const view=(poly.ownerSVGElement.getAttribute('viewBox')||'0 0 360 360').split(/\\s+/).map(Number);
    const cx=view[0]+view[2]/2;
    const cy=view[1]+view[3]/2;
    const valid=points.map(p=>Math.hypot(p.x-cx,p.y-cy)>2.5);
    const group=document.createElementNS(ns,'g');
    group.setAttribute('class','radar-series factor-series');
    group.setAttribute('data-product',poly.dataset.product||'');
    group.setAttribute('style',poly.getAttribute('style')||'');
    const addSegment=(a,b)=>{{
      ...
      line.setAttribute('class','radar-segment');
      group.appendChild(line);
    }};
    for(let i=0;i<points.length;i++){{
      const j=(i+1)%points.length;
      if(valid[i]&&valid[j]) addSegment(points[i],points[j]);
    }}
    points.forEach((p,i)=>{{
      if(!valid[i]) return;
