(function () {
  ...
  function cleanTableTitle(value) {
    return String(value || '')
      .replace(/^二、大区产业板块洞察\s*\|\s*/u, '')
      .replace(/^二、细分市场洞察\s*\|\s*/u, '')
      .replace(/^2、核心产品线—履带式挖掘机\s*/u, '')
      .replace(/^2、核心产品线—挖掘机\s*/u, '')
      .replace(/\s+/gu, ' ')
      .trim();
  }

  function renderBusinessTable(record) {
    const rows = Array.isArray(record.matrix_zh) ? record.matrix_zh : [];
    ...
    const roleTitle = {
      environment: 'Complete external-environment matrix',
      competition: 'Complete competitive-market matrix',
      portfolio: 'Complete product-portfolio matrix',
      roadmap: 'Complete improvement-roadmap table',
      sales_plan: 'Complete historical sales-target table',
      evidence_system: 'Complete information-loop table'
    }[record.role] || 'Complete business-data table';
    const tableTitle = cleanTableTitle(record.title);
    return `<details class="businessTablePanel"${language === 'zh' ? ' open' : ''} data-source-slide="${record.slide}" data-table-role="${esc(record.role)}">
      <summary>
        <h4 data-en="${esc(`${roleTitle} ${record.table_index || 1}`)}">${esc(tableTitle)}</h4>
        <span>${record.rows} × ${record.columns}</span>
      </summary>
      <div class="businessTableScroll">
        <table lang="zh-CN">
          <caption class="srOnly">${esc(tableTitle)}</caption>
          <thead><tr>${header.map((cell) => `<th scope="col">${esc(cell) || '&nbsp;'}</th>`).join('')}</tr></thead>
          <tbody>${body.map((row) => `<tr>${row.map((cell, index) => index === 0 ? `<th scope="row">${esc(cell) || '&nbsp;'}</th>` : `<td>${esc(cell) || '&nbsp;'}</td>`).join('')}</tr>`).join('')}</tbody>
        </table>
      </div>
    </details>`;
  }

  function renderBusinessTableGroup(records, titleZh, titleEn, leadZh, leadEn) {
    if (!records.length) return '';
    return `<section class="businessTableGroup">
      <div class="subsectionHead">
        <h3>${language === 'en' ? esc(titleEn) : esc(titleZh)}</h3>
        <p>${language === 'en' ? esc(leadEn) : esc(leadZh)}</p>
      </div>
      <div class="businessTableStack">${records.map(renderBusinessTable).join('')}</div>
    </section>`;
  }

  function renderOverviewBusinessTables(payload) {
    const ids = new Set(payload.overview || []);
    const records = (payload.records || []).filter((record) => ids.has(record.id));
    const byRole = (roles) => records.filter((record) => roles.includes(record.role));
    appendHtml('#market-demand-content', renderBusinessTableGroup(
      byRole(['environment']),
      '外部环境完整决策矩阵',
      'Complete external-environment decision matrix',
      '保留宏观变量、现象判断与产品动作的完整对应关系。',
      'The complete matrix retains the relationship between external variables, observed conditions and product actions.'
    ));
    appendHtml('#competition-content', renderBusinessTableGroup(
      byRole(['competition']),
      '品牌格局与标杆完整对比表',
      'Complete brand and benchmark tables',
      '包含品牌销量、市场份额、竞争梯队及微小挖和中大挖两套标杆体系。',
      'Includes brand volume, share, competitive tiers and the separate mini/compact and medium/large benchmark systems.'
    ));
    appendHtml('#portfolio-content', renderBusinessTableGroup(
      byRole(['portfolio']),
      '产品型谱完整矩阵',
      'Complete product-portfolio matrices',
      '按吨级保留常规尾、短尾、配置层级、覆盖状态和历史产品方向。',
      'Retains class, tail configuration, trim level, coverage status and historical product direction.'
    ));
    appendHtml('#actions-content', renderBusinessTableGroup(
      byRole(['roadmap', 'sales_plan', 'evidence_system']),
      '改进计划、销量目标与信息闭环台账',
      'Improvement, sales-target and information-loop tables',
      '历史计划按原年份展示；计划日期和目标不等同于当前完成状态。',
      'Historical plans remain tied to their original dates; planned dates and targets do not represent current completion.'
    ));
  }
  ...
  function renderEnvironmentDossiers(data) {
    const records = Array.isArray(data.external_environment) ? data.external_environment : [];
    if (!records.length) return '';
    const labels = language === 'en'
      ? {
        title: 'External environment: evidence, market implication and product response',
        lead: 'Five external forces are translated into product and market decisions. Historical figures retain their original period and are not presented as current 2026 facts.',
        evidence: 'Observed evidence',
        implication: 'Market and customer implication',
        action: 'Product and validation response',
        boundary: 'Current-use boundary'
      }
      : {
        title: '外部环境：事实、市场含义与产品响应',
        lead: '将五类外部变量完整转化为产品和市场动作；历史数字保留原时间口径，不作为 2026 年当前事实使用。',
        evidence: '观察事实',
        implication: '市场与客户含义',
        action: '产品与验证动作',
        boundary: '当前使用边界'
    ...
    const rows = records.map((item, index) => `
      <article class="environmentDossier">
        <header><span>${String(index + 1).padStart(2, '0')}</span><h4>${esc(text(item.factor))}</h4></header>
        <div class="environmentEvidence">
          <b>${labels.evidence}</b>
          <ul>${(item.metrics || []).map((metric) => `<li>${esc(text(metric))}</li>`).join('')}</ul>
        </div>
        <div>
          <b>${labels.implication}</b>
          <p>${esc(text(item.observation))}</p>
        </div>
        <div>
          <b>${labels.action}</b>
          <p>${esc(text(item.product_decision))}</p>
        </div>
        <footer><strong>${labels.boundary}</strong><span>${esc(text(item.status).replace(/资料时点/g, '历史时点').replace(/Source-period/g, 'Historical-period'))}</span></footer>
      </article>`).join('');
    return `
      <div class="subsectionHead frontMatterHead">
        <h3>${labels.title}</h3>
        <p>${labels.lead}</p>
      </div>
      <div class="environmentDossiers">${rows}</div>`;
  }
  ...
  function renderMarketDemand(data) {
    ...
    setHtml('#market-demand-content', `
      ...
      ${renderEnvironmentDossiers(data)}`);
  }
  ...
  function renderCompetitionEvolution(data) {
    const history = data.competition_history;
    if (!history || !Array.isArray(history.years)) return '';
    const groups = [
      ['north_american', language === 'en' ? 'North American brands' : '北美品牌'],
      ['japanese', language === 'en' ? 'Japanese brands' : '日系品牌'],
      ['european', language === 'en' ? 'European brands' : '欧洲品牌'],
      ['korean', language === 'en' ? 'Korean brands' : '韩系品牌'],
      ['chinese', language === 'en' ? 'Chinese brands' : '中国品牌']
    ];
    const rows = history.years.map((year, index) => `
      <div class="originMixRow">
        <b>${year}</b>
        <div class="originMixBar" aria-label="${year}">
          ${groups.map(([key, label]) => {
            const value = history.origin_mix[key][index];
            return `<span class="${key}" style="width:${value}%" title="${esc(label)} ${value}%">${value >= 3 ? `${value}%` : ''}</span>`;
          }).join('')}
        </div>
        <strong>${history.cr4[index]}%</strong>
      </div>`).join('');
    return `
      <div class="subsectionHead frontMatterHead">
        <h3>${language === 'en' ? 'Brand-origin structure and concentration, 2019-2023' : '2019–2023 品牌来源结构与集中度演变'}</h3>
        <p>${language === 'en' ? 'Origin share shows the long-term competitive base; CR4 measures how firmly the leading four brands control the market.' : '品牌来源份额反映长期竞争基本盘，CR4 用于观察前四品牌对市场的控制程度。'}</p>
      </div>
      <div class="competitionEvolution">
        <article class="originMixPanel">
          <header><b>${language === 'en' ? 'Origin mix' : '品牌来源结构'}</b><span>CR4</span></header>
          ${rows}
          <div class="originLegend">${groups.map(([key, label]) => `<span><i class="${key}"></i>${esc(label)}</span>`).join('')}</div>
        </article>
        <article class="evolutionReading">
          <b>${language === 'en' ? 'What changed' : '结构变化'}</b>
          <p>${esc(text(history.interpretation))}</p>
          <dl>
            <div><dt>${language === 'en' ? 'North American brands' : '北美品牌'}</dt><dd>${history.origin_mix.north_american[0]}% → ${history.origin_mix.north_american.at(-1)}%</dd></div>
            <div><dt>${language === 'en' ? 'Japanese brands' : '日系品牌'}</dt><dd>${history.origin_mix.japanese[0]}% → ${history.origin_mix.japanese.at(-1)}%</dd></div>
            <div><dt>CR4</dt><dd>${history.cr4[0]}% → ${history.cr4.at(-1)}%</dd></div>
          </dl>
          <p>${language === 'en'
            ? 'The implication is a two-sided challenge: new entrants can gain space, but product improvements must be reinforced by dealer coverage, parts, finance and residual-value confidence to become durable share.'
            : '这意味着市场并非没有新品牌空间，但产品改进必须与经销覆盖、配件、金融和残值信心同步建设，才能把一次性成交变成稳定份额。'}</p>
        </article>
      </div>`;
  }

  function renderBrandMovementPanel(items, title, concentrationLabel) {
    const ranked = items
      .filter((item) => Number.isFinite(item.y2025_units))
      .sort((a, b) => b.y2025_units - a.y2025_units);
    const selected = ranked.slice(0, 10);
    const xcmg = ranked.find((item) => item.brand === 'XCMG');
    if (xcmg && !selected.includes(xcmg)) selected.push(xcmg);
    const rows = selected.map((item) => {
      const delta = item.y2024_units > 0 ? (item.y2025_units - item.y2024_units) / item.y2024_units : null;
      const deltaClass = delta == null ? '' : delta > 0 ? 'up' : delta < 0 ? 'down' : 'flat';
      const deltaLabel = delta == null ? '—' : `${delta > 0 ? '+' : ''}${(delta * 100).toFixed(1)}%`;
      return `<div class="brandMovementRow${item.brand === 'XCMG' ? ' xcmg' : ''}">
        <b>${esc(item.brand)}</b>
        <span>${formatNumber(item.y2024_units)} <small>${item.y2024_share}%</small></span>
        <span>${formatNumber(item.y2025_units)} <small>${item.y2025_share}%</small></span>
        <strong class="${deltaClass}">${deltaLabel}</strong>
      </div>`;
    }).join('');
    return `<article class="brandMovementPanel">
      <header><div><h4>${esc(title)}</h4><span>${esc(concentrationLabel)}</span></div></header>
      <div class="brandMovementHeader"><b>${language === 'en' ? 'Brand' : '品牌'}</b><b>2024</b><b>2025</b><b>${language === 'en' ? 'Change' : '变化'}</b></div>
      ${rows}
    </article>`;
  }

  function renderBrandMovement(data) {
    const volume = data.brand_volume_2024_2025;
    if (!volume) return '';
    return `
      <div class="subsectionHead">
        <h3>${language === 'en' ? 'Brand movement from 2024 to 2025' : '2024–2025 品牌销量与份额变化'}</h3>
        <p>${language === 'en' ? 'Unit volume and share are shown together so a brand is not judged by rank alone. Complete source-period brand tables follow below.' : '同时显示销量与份额，避免只凭排名判断；完整品牌明细表在本节下方继续展开。'}</p>
      </div>
      <div class="brandMovementGrid">
        ${renderBrandMovementPanel(volume.compact, copy.competitionCompact, copy.compactConcentration)}
        ${renderBrandMovementPanel(volume.mid_large, copy.competitionLarge, copy.largeConcentration)}
      </div>`;
  }

  function renderBenchmarkDossiers(data) {
    const records = Array.isArray(data.benchmark_systems) ? data.benchmark_systems : [];
    if (!records.length) return '';
    const labels = language === 'en'
      ? [
        ...
        ['Implication for XCMG', 'xcmg_implication']
      ]
      : [
        ['产品体系', 'product'],
        ['客户基础', 'customer'],
        ...
        ['商务体系', 'commercial'],
        ['服务体系', 'service'],
        ['对 XCMG 的含义', 'xcmg_implication']
      ];
    return `
      <div class="benchmarkDossiers">
        ${records.map((item) => `<article class="benchmarkDossier">
          <header><div><span>${esc(text(item.scope))}</span><h4>${esc(item.brand)}</h4></div><strong>${language === 'en' ? 'Strategic benchmark' : '战略标杆'}</strong></header>
          <div>${labels.map(([label, key]) => `<section><b>${label}</b><p>${esc(text(item[key]))}</p></section>`).join('')}</div>
          <footer>${esc(text(item.status).replace(/资料时点/g, '历史时点').replace(/Source-period/g, 'Historical-period'))}</footer>
        </article>`).join('')}
      </div>`;
  }
  ...
  function renderCompetition(data) {
    ...
    setHtml('#competition-content', `
      ${renderCompetitionEvolution(data)}
      ...
      ${renderBenchmarkDossiers(data)}`);
  }

  function renderClassCase(data) {
    const item = data.class_case_1_2t;
    if (!item) return '';
    const customerMix = item.customer_mix.map((segment) => `<div><b>${esc(text(segment.segment))}</b><i><em style="width:${segment.share}%"></em></i><strong>${segment.share}%</strong></div>`).join('');
    const factList = item.transport.facts.map((fact) => `<li>${esc(text(fact))}</li>`).join('');
    const taskList = item.landscaping.tasks.map((task) => `<li>${esc(text(task))}</li>`).join('');
    const requirementList = item.landscaping.requirements.map((requirement) => `<li>${esc(text(requirement))}</li>`).join('');
    return `
      <div class="subsectionHead frontMatterHead">
        <h3>${esc(text(item.title))}</h3>
        <p>${language === 'en' ? 'A complete mini-excavator case connecting market structure, transport economics, customer mix and a real landscaping task chain.' : '用一个完整微挖案例，把市场结构、运输经济性、客户构成和园林任务链连接起来。'}</p>
      </div>
      <section class="classCaseStudy">
        <div class="classCaseTop">
          <figure class="modelHeatmapFigure">
            <img src="${esc(item.model_heatmap_image)}" alt="${language === 'en' ? '1-2 t model and brand comparison' : '1–2 吨品牌与机型对比'}">
            <figcaption><b>${language === 'en' ? 'Model and configuration structure' : '机型与配置结构'}</b><p>${esc(text(item.model_findings))}</p></figcaption>
          </figure>
          <article class="customerMixPanel">
            <h4>${language === 'en' ? 'Primary customer mix' : '主要客户构成'}</h4>
            <div>${customerMix}</div>
            <p>${language === 'en'
              ? 'Landscaping, small contractors and rental together account for 90% of the documented mix. Product definition therefore has to balance easy transport, novice-operator usability, attachment breadth and low service risk.'
              : '景观绿化、小型承包商和租赁客户合计占记录结构的 90%。产品定义必须同时兼顾运输便利、非专业机手适应性、属具广度和低停机风险。'}</p>
          </article>
        </div>
        <div class="classCaseTransport">
          <div class="classCaseImages two">
            ${item.transport.images.map((image, index) => `<figure><img src="${esc(image)}" alt="${language === 'en' ? 'Mini excavator transport combination' : '微挖运输组合'}"><figcaption>${index === 0 ? (language === 'en' ? 'Trailer transport envelope' : '拖车运输边界') : (language === 'en' ? 'Pickup and trailer combination' : '皮卡与拖车组合')}</figcaption></figure>`).join('')}
          </div>
          <article>
            <h4>${language === 'en' ? 'Transport boundary and equipped mass' : '运输边界与装配质量'}</h4>
            <ul>${factList}</ul>
            <p>${esc(text(item.transport.conclusion))}</p>
          </article>
        </div>
        <div class="classCaseLandscape">
          <div class="classCaseImages three">
            ${item.landscaping.images.map((image, index) => `<figure><img src="${esc(image)}" alt="${language === 'en' ? 'Landscaping task' : '园林作业任务'}"><figcaption>${[
              language === 'en' ? 'Trenching and earth shaping' : '沟槽与地形修整',
              language === 'en' ? 'Material and plant handling' : '物料与苗木搬运',
              language === 'en' ? 'Precision work near finished surfaces' : '硬化地面附近精细作业'
            ][index]}</figcaption></figure>`).join('')}
          </div>
          <div class="landscapeReading">
            <article><h4>${language === 'en' ? 'Task chain' : '施工任务链'}</h4><ol>${taskList}</ol></article>
            <article><h4>${language === 'en' ? 'Machine requirements' : '整机能力要求'}</h4><ul>${requirementList}</ul></article>
          </div>
          <div class="caseConclusion"><b>${language === 'en' ? 'Product conclusion' : '产品结论'}</b><p>${esc(text(item.landscaping.conclusion))}</p><a href="excavator-1-2t.html">${language === 'en' ? 'Open the 1-2 t benchmark →' : '进入 1–2 吨产品对标 →'}</a></div>
        </div>
      </section>`;
  }
  ...
  async function init() {
    ...
    try {
      const [overviewResponse, tableResponse] = await Promise.all([
        fetch('data/ppt-insights/excavator-market-overview.json', {cache: 'no-store'}),
        fetch('data/ppt-insights/ppt-business-tables.json', {cache: 'no-store'})
      ]);
      if (!overviewResponse.ok) throw new Error(`Overview HTTP ${overviewResponse.status}`);
      if (!tableResponse.ok) throw new Error(`Business tables HTTP ${tableResponse.status}`);
      ...
      renderActions(data);
      renderOverviewBusinessTables(tableData);
